...
- Resolve PR metadata first, then write intake artifact.
- When intake `cqlPaths` is empty and PAT fallback is allowed, run `scripts/ado_fallback.py enrich-intake` to enrich changed CQL paths.
