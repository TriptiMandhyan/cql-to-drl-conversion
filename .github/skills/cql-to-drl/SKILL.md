...
5. Then compare with matching reference DRL patterns when available for:
...
7. Enforce marker fact contract for EMR fire-once semantics:
...
8. Enforce fact modeling contract aligned to platform DRL conventions:
...
9. Enforce EMR output semantics:
...
10. Enforce semantic-preservation rules:
...
11. Emit DRL in package structure expected by target repository and preserve shared-file handling conventions.
12. Generate mapping document that links each CQL clause to DRL rule ids.
13. In mapping notes, explicitly record any intentional deviation from source CQL semantics and why equivalence is preserved.
14. In mapping notes, explicitly record any intentional deviation from reference DRL structure, including attribution strategy, naming, salience, shared dependencies, or accumulate direction.
15. In mapping notes for any `option_id = all` clause, list the concrete option-code set used for EMR fan-out.
16. Flag unsupported constructs and assumptions.
17. Call MCP tool `local-python.validate_conversion_artifacts(ticket_id)` and fix violations before finalizing outputs.
